...
sm_category%>%
  group_by(category)%>%
  summarize(humor = humor, p = count / sum(count))
...
data%>%
  ggplot()+
  geom_bar(aes(chrlength))+
  labs(title = 'Length of the sentences')

# Humor and Not Humor length of the sentences
data%>%
  ggplot()+
  geom_boxplot(aes(humor, chrlength))+
  labs(title = 'Humor and Not Humor length of the sentences')
