...
ggplot(data)+geom_bar(aes(Age,fill=Drug),position = 'identity')+labs(title='Drugs in each ages')
data%>%group_by(Drug,BP)%>%summarize(count=n())%>%ggplot()+geom_col(aes(Drug,BP,fill=BP),position='dodge')+labs(title='BP of each Drugs')
...
new_data%>%group_by(Age,BP)%>%summarize(count=n())%>%ggplot()+geom_col(aes(Age,count,fill=BP),position='dodge')+
labs(title='Blood pressure in each age')
new_data%>%group_by(Age,Cholesterol)%>%summarize(count=n())%>%ggplot()+geom_col(aes(Age,count,fill=Cholesterol),position='dodge')+
labs(title='Cholesterol in each age')

#Plot the Sodium to potassium Ration in Blood with each Sex and BP
ggplot(data)+geom_boxplot(aes(Sex,Na_to_K,fill=Sex),position = 'identity')+labs(title='Sodium to potassium Ration in Blood at each Sex')
ggplot(data)+geom_boxplot(aes(BP,Na_to_K,fill=BP),position = 'identity')+labs(title='Sodium to potassium Ration in Blood at each BP')
