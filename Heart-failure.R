...
#Data Visualize
ggplot(data)+geom_bar(aes(DEATH_EVENT,fill=DEATH_EVENT),position='identity')+scale_fill_brewer()+theme_dark()+labs(title='Count of Death event')
dt<-cor(data)
...
n=1
for(i in lst){
  print(ggplot(data)+geom_boxplot(aes(DEATH_EVENT,i),fill=lstcolor[n],alpha=0.5)+ylab(label = names(lst)[n]))
    n=n+1
...
model_data<-data.frame(accuracy=c(svm_acc[[2]],dt_acc,knn_acc,nn_acc[[2]],rf_acc,xgb_acc),
           model=c('SVM','DT','Knn',
                   'NN','RF','Xgb'))
model_data$accuracy<-(100*round(model_data$accuracy,3))
model_data
ggplot(model_data)+geom_bar(aes(model,accuracy,fill=accuracy),stat = 'identity')+
